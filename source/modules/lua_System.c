...
static int lua_getdir (lua_State *L) {
	if (lua_gettop(l) != 1) return luaL_error(l, "wrong number of arguments");
	const char *path = luaL_checkstring(l, 1);
	BrowserData dir = dirIter(path);
	BrowserData* buffer = pushDir(l);
	*buffer = dir;
    return 1;
}
static int lua_getnumdir (lua_State *L) {
	if (lua_gettop(l) != 0) return luaL_error(l, "wrong number of arguments");
	BrowserData dir = *toDir(l, 1);
	lua_pushnumber(l, dir.maxbrow);
    return 1;
}
static int lua_getname (lua_State *L) {
	if (lua_gettop(l) != 1) return luaL_error(l, "wrong number of arguments");
	BrowserData dir = *toDir(l, 1);
	lua_pushstring(l, dirGetItem(dir).file);
    return 1;
}
static int lua_gettype (lua_State *L) {
	if (lua_gettop(l) != 1) return luaL_error(l, "wrong number of arguments");
	BrowserData dir = *toDir(l, 1);
	lua_pushnumber(l, dirGetItem(dir).type);
    return 1;
}
static int lua_isext (lua_State *L) {
	if (lua_gettop(l) != 2) return luaL_error(l, "wrong number of arguments");
	BrowserData dir = *toDir(l, 1);
	const char *exten = luaL_checkstring(l, 2);
	char *res = strstr(dirGetItem(dir).ext, exten);
	if (res) {
		lua_pushnumber(l, 1);
	} else {
		lua_pushnumber(l, 0);
