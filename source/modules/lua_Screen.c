...
static int lua_screenBlitSurface(lua_State *l) {
	...
	apply_surface(luaL_checkint(l, 1), luaL_checkint(l, 2), *toImage(l, 3), screen, NULL);
	return 1;
}
...
static int lua_screenColor(lua_State *l) {
	...
	int color;
	if (arg == 3) color = SDL_MapRGB(screen->format, red, g, b);
	if (arg == 4) { int a = luaL_checkint(l, 4); color = SDL_MapRGBA(screen->format, red, g, b, a); }
...
static int lua_screenfillCircle(lua_State *l) {
	...
	if (arg == 4) {
		Draw_FillCircle(screen, x, y, rad, color);
	} else {
...
static int lua_screenCircle(lua_State *l) {
	...
	int color = luaL_checkint(l, 4);
	if (arg == 4) {
		Draw_Circle(screen, x, y, rad, color);
	} else {
...
static int lua_screenfillEllipse(lua_State *l) {
	...
	int color = luaL_checkint(l, 5);
	if (arg == 5) {
		Draw_FillEllipse(screen, x, y, w, h, color);
	} else {
...
static int lua_screenEllipse(lua_State *l) {
	...
	int color = luaL_checkint(l, 5);
	if (arg == 5) {
		Draw_Ellipse(screen, x, y, w, h, color);
	} else {
...
static int lua_screenfillRound(lua_State *l) {
	...
	if (arg == 6) {
		Draw_FillRound(screen, x, y, w, h, ron, color);
	} else {
...
static int lua_screenRound(lua_State *l) {
	...
	int color = luaL_checkint(l, 6);
	if (arg == 6) {
		Draw_Round(screen, x, y, w, h, ron, color); 
	} else {
...
static int lua_imageBlit(lua_State *l) {
	...
	if (lua_gettop(l) == 3) {
		...
		apply_surface(x,y, image, screen, &clip);
	} else if (lua_gettop(l) == 8) {
		SDL_Rect clip;
		clip.x = luaL_checkint(l, 4);
		clip.y = luaL_checkint(l, 5);
